//! Criterion and reference-host percentile measurements for `RNNoise` inference.
...
const P99_LIMIT: Duration = Duration::from_micros(750);
const TIMING_SAMPLES: usize = 20_000;

fn model_frame(criterion: &mut Criterion) {
    let factory = or_exit(RnnoiseFactory::new(), "benchmark factory creation");
    let input = signal_frame();
    let p99 = measure_p99(&factory, &input);
    eprintln!("Noire RNNoise model p99 over {TIMING_SAMPLES} frames: {p99:?}");
    assert!(
        p99 <= P99_LIMIT,
        "RNNoise model p99 {p99:?} exceeds {P99_LIMIT:?}"
    );

    let mut model = or_exit(factory.create(), "benchmark model creation");
    let mut output = [0.0; RNNOISE_FRAME_SAMPLES];
    let mut group = criterion.benchmark_group("rnnoise");
...
fn measure_p99(factory: &RnnoiseFactory, input: &[f32; RNNOISE_FRAME_SAMPLES]) -> Duration {
